...
1. **Sort every input into its tier** (per `source-hierarchy.md`). Produce a short source map: which file is Tier 1, 2, or 3, and what each one is good for. Note any source marked sensitive (e.g. a file classified "Sensitive — Internal Use Only") and hold its numbers for author clearance.
