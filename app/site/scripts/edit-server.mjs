...
/* Incremental narration: re-synthesize only the changed/dirty blocks of a paper
   and re-stitch the full MP3. Reads the freshly-saved JSON (the editor saves
   before calling this, persisting the dirty flags), regenerates via the shared
   segmented engine, and writes the updated segment manifest + durations back. */
async function handleRegenerateAudio(body, res) {
  if (!EDIT_ENABLED) return sendJSON(res, 403, { error: 'Editing is disabled in this environment.' });
  try {
    const id = String(body.id || '');
    const locale = String(body.locale || 'en');
    if (!/^[a-z0-9]+$/.test(id)) return sendJSON(res, 400, { error: 'Invalid id' });
    if (!/^[a-z]{2,3}$/.test(locale)) return sendJSON(res, 400, { error: 'Invalid locale' });
    const contentPath = resolve(SITE, 'data/papers/' + id + '.' + locale + '.json');
    if (!existsSync(contentPath)) return sendJSON(res, 404, { error: 'No such paper file.' });
    const content = JSON.parse(readFileSync(contentPath, 'utf8'));
    if (!content.audio || !content.audio.src) return sendJSON(res, 400, { error: 'Paper has no audio.src.' });
    const { isSegmented, regenerateSegmentedNarration } = await import('./lib/audio-segments.mjs');
    if (!isSegmented(content)) return sendJSON(res, 400, { error: 'Paper is not segmented yet (its blocks need stable ids).' });
    // Optional targeted run: a single seg id or an array of them.
    const only = body.seg ? [String(body.seg)] : (Array.isArray(body.only) ? body.only.map(String) : null);
    const r = await regenerateSegmentedNarration(content, contentPath, {
      locale, siteRoot: SITE, force: !!body.force, only, stitchOnly: !!body.stitch,
      log: (m) => console.log('[regen ' + id + '.' + locale + '] ' + m),
    });
    console.log('Regenerated audio ' + id + '.' + locale + ': ' + r.regenerated + ' changed, ' + r.reused + ' reused');
...
async function handleTranslatePaper(body, res) {
  ...
  try {
    ...
    /* Never blow away human-reviewed French. A reviewed target is authoritative;
       a hard overwrite must be an explicit, deliberate choice (body.force), and
       the right path for EN changes is per-section drift re-translation that
       leaves reviewed blocks intact (see docs/fr-style-guide.md §6). */
    const tgtPath = resolve(SITE, 'data/papers/' + id + '.' + tgt + '.json');
    if (!body.force && existsSync(tgtPath)) {
      try {
        const existing = JSON.parse(readFileSync(tgtPath, 'utf8'));
        if (existing && existing.translation_status === 'reviewed') {
          return sendJSON(res, 409, { error: 'Target ' + id + '.' + tgt + ' is human-reviewed (translation_status:"reviewed"). Refusing hard overwrite. Re-translate only the drifted blocks, or pass force:true to override.' });
        }
      } catch (_) {}
    }
...
const server = createServer((req, res) => {
  ...
  if (req.method === 'POST' && req.url === '/api/regenerate-audio') { readBody(req, (b) => handleRegenerateAudio(b, res)); return; }
