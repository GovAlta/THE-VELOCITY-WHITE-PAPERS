...
const site = existsSync(resolve(SITE, 'data/site.json')) ? JSON.parse(readFileSync(resolve(SITE, 'data/site.json'), 'utf8')) : {};
const LOCALES = (site.locales || []).map((l) => l.code);
if (!LOCALES.length) LOCALES.push('en', 'fr');
...
for (const id of ids) {
  const secByLoc = {};
  for (const loc of LOCALES) secByLoc[loc] = measure(id, loc);
  // reading_min is a shared display field; align it to the default-locale narration length.
  const readMin = secByLoc[DEF] ? Math.max(1, Math.round(secByLoc[DEF] / 60)) : null;
  rows.push({ id, secByLoc, readMin });
  ...
  for (const loc of LOCALES) {
    ...
    let changed = false;
    const sec = secByLoc[loc];
    if (sec != null) {
      if (!doc.audio) doc.audio = {};
      if (doc.audio.duration_sec !== sec) { doc.audio.duration_sec = sec; changed = true; }
    }
...
console.log('id'.padEnd(8) + LOCALES.map((l) => l.toUpperCase().padEnd(10)).join('') + 'read_min');
for (const r of rows) console.log(r.id.padEnd(8) + LOCALES.map((l) => fmt(r.secByLoc[l]).padEnd(10)).join('') + (r.readMin ?? '—'));
for (const loc of LOCALES) {
  const total = rows.reduce((a, r) => a + (r.secByLoc[loc] || 0), 0);
  console.log(loc.toUpperCase() + ' total narration: ' + Math.floor(total / 3600) + 'h ' + Math.round((total % 3600) / 60) + 'm across ' + rows.filter((r) => r.secByLoc[loc]).length + ' papers.');
}
