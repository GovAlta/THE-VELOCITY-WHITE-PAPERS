...
/* Running token usage across all vertexMessages calls this process, for cost
   tracking. Additive and optional — callers that don't care can ignore it. */
const _usage = { input: 0, output: 0, calls: 0 };
export function getUsage() { return { ..._usage }; }
export function resetUsage() { _usage.input = 0; _usage.output = 0; _usage.calls = 0; }
...
export async function vertexMessages({ model, system, messages, max_tokens }) {
  ...
  if (json && json.usage) { _usage.input += json.usage.input_tokens || 0; _usage.output += json.usage.output_tokens || 0; _usage.calls += 1; }
  return json;
}
...
/* Build the binding Canadian-French guidance from data/fr-lexicon.json (the
   standard derived from the Government of Alberta francophone reviewers). Falls
   back to a short inline note if the file can't be read. */
let _frLex = null;
function frLexiconPrompt() {
  if (_frLex == null) {
    try {
      const p = new URL('../../data/fr-lexicon.json', import.meta.url);
      const lex = JSON.parse(readFileSync(p, 'utf8'));
      const terms = (lex.terms || []).map(t => '"' + t.en + '" → "' + t.fr + '"' + (t.avoid ? ' (NOT ' + t.avoid.map(a => '"' + a + '"').join('/') + ')' : '')).join('; ');
      const typo = Object.values(lex.typography || {}).join(' ');
      _frLex = 'Write Canadian/Québécois public-administration French (OQLF-aligned). BINDING TERMS: ' + terms
        + ' TYPOGRAPHY: ' + typo
        + ' The English "no em dashes / no not-X-but-Y" rule does NOT apply to French. Plain, clear, calm institutional voice; split long sentences; soften incidental war/siege metaphors.';
    } catch (e) { _frLex = 'Use Canadian French public-administration conventions: "livre blanc" for white paper; "contrôles de cybersécurité" (never "cybernétiques"); "appuyé par l’IA" not "piloté"; spell out "millions/milliards de dollars" (no M$/G$); curly apostrophes and « guillemets ».'; }
  }
  return _frLex;
}
...
export async function translateItems({ items, sourceLocale, targetLocale, glossary, model, tier }) {
  ...
  const frConventions = targetLocale === 'fr' ? ' ' + frLexiconPrompt() + ' ' : ' ';
