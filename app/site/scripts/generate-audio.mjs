...
/* generate-audio.mjs — produce MP3 narration files per paper and per slide.
   ...
   Usage:
     ...
     node scripts/generate-audio.mjs --longform-only    # only full-paper narration (skip slides)
*/
...
import { isSegmented, regenerateSegmentedNarration } from './lib/audio-segments.mjs';
...
async function processPaper(id, locale) {
  ...
  const slideJobs = longformOnly ? [] : collectSlideJobs(content);
  let gen = 0, cached = 0;
  ...
  if (!tldrOnly && !noLongform && content.audio && content.audio.src) {
    if (isSegmented(content)) {
      // Incremental: one MP3 per unit, reused unless changed, then stitched.
      const r = await regenerateSegmentedNarration(content, contentPath, {
        locale, siteRoot: SITE_ROOT, force,
        log: (m) => console.log('[seg] ' + tag + ' ' + m),
      });
      gen += r.regenerated; cached += r.reused;
      console.log('[stitch] ' + tag + ' → ' + content.audio.src + '  (' + r.regenerated + ' regenerated, ' + r.reused + ' reused, ' + r.duration_sec + 's)');
    } else {
      // Legacy monolithic narration for papers not yet migrated to segments.
