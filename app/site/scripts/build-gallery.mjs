...
for (const p of papers) {
  ...
  // Match FR blocks to EN by STABLE IDENTITY (figure number / YouTube id), not
  // by array index. EN and FR `blocks` arrays are not index-aligned (FR may
  // split or reorder prose), so index matching made ~40% of figures fall back
  // to the English title in French mode. fno/url is stable across locales.
  const frFigByFno = new Map();
  const frVidByYt = new Map();
  frBlocks.forEach((fb) => {
    if (fb && fb.type === 'figure' && fb.fno) frFigByFno.set(String(fb.fno), fb);
    if (fb && fb.type === 'youtube' && fb.url) { const y = youtubeId(fb.url); if (y) frVidByYt.set(y, fb); }
  });

  (doc.blocks || []).forEach((b, i) => {
    let fb = null;
    if (b.type === 'figure') {
      fb = (b.fno && frFigByFno.get(String(b.fno))) || (frBlocks[i] && frBlocks[i].type === 'figure' ? frBlocks[i] : null);
    } else if (b.type === 'youtube') {
      const y = youtubeId(b.url);
      fb = (y && frVidByYt.get(y)) || (frBlocks[i] && frBlocks[i].type === 'youtube' ? frBlocks[i] : null);
    }
