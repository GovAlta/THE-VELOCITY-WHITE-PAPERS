...
(function () {
  ...
  function localeList() { return (W.VWStore && W.VWStore.site && W.VWStore.site.locales) || [{ code: 'en' }]; }
  function localeMeta(loc) { return localeList().find(l => l.code === loc) || {}; }
  function ogLocale(loc) { return localeMeta(loc).og_locale || 'en_CA'; }
  function langCode(loc) { return localeMeta(loc).lang_code || 'en-CA'; }
  function defaultLoc() { return (W.VWStore && W.VWStore.site && W.VWStore.site.default_locale) || 'en'; }
  ...
  V.setSitePage = function (pageId, pageTitle, descriptionOverride) {
    ...
    setProperty('og:locale', ogLocale(d.locale));
    ...
    const pagePath = (code) => SITE_URL_BASE + '/' + (code === defaultLoc() ? seg : (seg ? seg + '/' + code : code));
    for (const l of localeList()) setLink('alternate', pagePath(l.code), l.code);
    setLink('alternate', pagePath(defaultLoc()), 'x-default');
  ...
  V.setPaper = function (paper) {
    ...
    setLink('canonical', url);
    clearHreflangs();
    const paperPath = (code) => SITE_URL_BASE + '/paper/' + paper.id + (code === defaultLoc() ? '' : '/' + code);
    for (const l of localeList()) setLink('alternate', paperPath(l.code), l.code);
    setLink('alternate', paperPath(defaultLoc()), 'x-default');
    ...
    template: `
      <nav class="civic-nav" v-if="store.site" :class="{ 'drawer-open': drawerOpen }">
        <a class="brand" href="/"
           ...
           :aria-label="((store.t.ui && store.t.ui.home_prefix) || 'Home, ') + store.t.title">
        ...
        <div class="nav-trail">
          <div class="locale-switch" role="group"
               :aria-label="(store.t.ui && store.t.ui.language_selection) || 'Language selection'">
            <button v-for="l in store.site.locales"
                    :key="l.code"
                    :class="{ on: store.locale === l.code }"
                    :aria-pressed="store.locale === l.code ? 'true' : 'false'"
                    :aria-label="((store.t.ui && store.t.ui.switch_to) || 'Switch to {name}').replace('{name}', l.name || l.name_en || l.label)"
                    :lang="l.code"
        ...
        <button class="nav-hamburger"
                ...
                :aria-label="drawerOpen
                  ? ((store.t.ui && store.t.ui.close_menu) || 'Close menu')
                  : ((store.t.ui && store.t.ui.open_menu) || 'Open menu')"
                @click="toggleDrawer">
        ...
        <div id="nav-drawer" class="nav-drawer" v-show="drawerOpen" role="navigation"
             :aria-label="(store.t.ui && store.t.ui.primary_menu) || 'Primary menu'">
...
(function () {
  ...
  window.VWComponents['paper-card'] = {
    ...
    computed: {
      ...
      statusLabel() {
        const map = this.store.t.ui && this.store.t.ui.status_labels;
        return (map && map[this.paper.status]) || this.paper.status;
      },
      paperLabel() {
        const parts = [];
...
(function () {
  ...
  window.VWComponents['index-table'] = {
    ...
    methods: {
      ...
      statusLabel(s) { const m = this.store.t.ui && this.store.t.ui.status_labels; return (m && m[s]) || s; },
      async downloadRow(p) {
        const loc = this.store.locale || 'en';
        try {
          const doc = await window.VWExport.fetchPaper(p.id, loc);
          window.VWExport.download(p.id + '.' + loc + '.md', 'text/markdown', window.VWExport.blocksToMarkdown(doc, loc));
        } catch (e) {  }
      },
      async downloadAll() {
        ...
        catch (e) { this.dlErr = (this.store.t.ui && this.store.t.ui.download_failed) || 'Download failed.'; }
      ...
      <section class="civic-index">
        <div class="index-head">
          ...
          <button class="idx-dlall" :disabled="dlBusy" @click="downloadAll"
                  :title="(store.t.ui && store.t.ui.download_all_tooltip) || 'Download every paper as JSON + Markdown (no media)'">
            <span aria-hidden="true">⤓</span>
            {{ dlBusy ? ((store.t.ui && store.t.ui.preparing) || 'Preparing…') : ((store.t.ui && store.t.ui.download_all) || 'Download all (.zip)') }}
          </button>
        </div>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th>{{ (store.t.ui && store.t.ui.get) || 'Get' }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(p, i) in filtered" :key="p.id" @click="$emit('open', p.id)">
              ...
              <td :class="statusClass(p)" data-label="Status">{{ statusLabel(p.status) }}</td>
              <td class="idx-get" data-label="Get" @click.stop>
                <button class="idx-dl" @click="downloadRow(p)"
                        :title="(store.t.ui && store.t.ui.download_markdown) || 'Download Markdown'"
                        :aria-label="((store.t.ui && store.t.ui.download_markdown) || 'Download Markdown') + ': ' + titleOf(p)">
...
(function () {
  ...
  window.VWComponents['block-renderer'] = {
    ...
    setup() { return { store: window.VWStore }; },
    computed: {
      tagsLabel() { return (this.store && this.store.t && this.store.t.ui && this.store.t.ui.tags) || 'Tags'; },
    ...
    methods: {
      ...
      downloadMD() {
        window.VWExport.download(
          ...
          'text/markdown',
          window.VWExport.blocksToMarkdown(this.paper, this.store.locale || 'en')
        );
      },
    },
    template: `
      <div class="paper-downloads">
        <span class="pd-lbl">{{ (store.t.ui && store.t.ui.download) || 'Download' }}</span>
      ...
      async load() {
        ...
        try {
          ...
          if (window.VWA11y && this.content) {
            const prefix = (this.store.t.ui && this.store.t.ui.paper_loaded) || 'Paper loaded: ';
...
(function () {
  ...
  window.VWSetLocale = switchLocale;   
  ...
  function parsePath() {
    ...
    if (parts.length && looksLikeLocale(parts[0])) {
      _localeHint = parts.shift().toLowerCase();                 
    } else if (parts.length > 1 && looksLikeLocale(parts[parts.length - 1])) {
      _localeHint = parts.pop().toLowerCase();                   
    }
  ...
  function switchLocale(code) {
    ...
    const prefixed = localizedPath(pathFor(target));   
  ...
  const app = createApp({
    setup() {
      ...
      function setLocale(code) { switchLocale(code); }   
