...
## Localization and language links

The site is multilingual and **fully static** — the language is resolved entirely
in the browser, so there is no server to configure. Everything keys off the
`locales` array and `default_locale` in **`data/site.json`**; adding a language is
a data operation (see **`docs/i18n-adding-a-language.md`** for the full playbook,
and **`docs/i18n-cost-ledger.md`** for the per-language cost model).

**How the active language is chosen** (highest precedence first):

1. **A locale in the URL** — see the link forms below. A shared link always opens
   in its own language, overriding any prior preference.
2. **The saved preference** — the `vw_locale` cookie / `localStorage` value, set
   whenever the reader picks a language or opens a locale link.
3. **`default_locale`** from `site.json` (currently `en`).

**Permanent, shareable language links** (the point of static localization):

| Form | Example | Notes |
|---|---|---|
| Leading prefix (canonical) | `/es/`, `/es/paper/<id>`, `/fr/glossary`, `/en/about` | The form to share and link to. |
| Trailing suffix (hreflang) | `/paper/<id>/fr`, `/about/fr` | The crawler/`hreflang` form; also honored. |

Opening either form sets `store.locale`, `<html lang>` (and `dir`, so RTL
languages flip automatically), writes the `vw_locale` cookie **and** `localStorage`,
and renders that language. An unknown code (e.g. `/xx/`) falls back to
`default_locale`. Detection lives in `app.js` — `parsePath()` strips the locale
segment and `applyLocaleHint()` applies it, on first load and on every navigation.

**One canonical URL form — always prefixed.** Every page has a single URL per
language, always carrying the locale: `/en/paper/x`, `/fr/paper/x`, `/es/paper/x`
— including the default locale (there is no prefix-less twin). `localizedPath()`
prefixes every in-app navigation and the language switcher, and on load the
address bar is **normalized** to the prefixed form: a bare `/paper/x` becomes
`/en/paper/x` (or the reader's stored language), and `/` becomes `/en/`. So the
language follows you through the whole session and every link is unambiguous and
shareable. Precedence for the active locale: **URL prefix > `vw_locale`
cookie/localStorage > `default_locale`** — a first-time visitor with no cookie
lands on `/en/`. (SEO note: crawler-facing canonical/hreflang and the prerendered
shells still emit the clean/​suffix forms; aligning those to the prefix form is
the outstanding SEO task — the runtime user-facing routing is single-form today.)

**Why it works on GitHub Pages (no server rewrites):** a prefixed path like
`/es/paper/x` has no prerendered file, so Pages serves `404.html`, which redirects
to `/?redirect=/es/paper/x`; the boot script in `index.html` restores the clean
URL before the app mounts, and the router applies the locale. Prerendered SEO
shells and per-locale `sitemap`/`hreflang` currently cover `en`+`fr`; generating
them for additional locales is the one remaining SEO task (the runtime deep-link
works for every declared locale today regardless).

**Missing translations degrade gracefully:** `applyLocale` deep-merges a locale's
`i18n` bag over the default locale's, so any string a language hasn't translated
yet falls back to the default rather than rendering blank — a language can ship
incrementally. `npm run eval` includes `check-locale-completeness`, which reports
per-locale UI coverage and hard-fails on structural gaps.
