...
(function () {
  ...
  window.VWComponents['paper-detail'] = {
    ...
    computed: {
      /* A paper's repositories, derived from data/repos.json (store.repos) by
         paper id, so a paper can carry more than one (e.g. Pronghorn → RED +
         BLUE). Falls back to the legacy single `repo` field if no repos.json
         entry references this paper. Quantity and names are not hardcoded. */
      paperRepos() {
        const id = this.paper && this.paper.id;
        const linked = (this.store.repos || []).filter(r => r.paper === id);
        if (linked.length) return linked.map(r => ({ name: r.name, url: r.url, available: r.available !== false, private: !!r.private }));
        if (this.paper && this.paper.repo) return [{ name: String(this.paper.repo).replace('https://github.com/', ''), url: this.paper.repo, available: true, private: false }];
        return [];
      },
      translationLabel() {
    ...
    methods: {
      ...
      /* Per-block narration segment lookups for the in-bar <block-audio>. */
      segSrc(bid) { const m = this.edit && this.edit.segFor && this.edit.segFor(bid); return m ? m.src : ''; },
      segDur(bid) { const m = this.edit && this.edit.segFor && this.edit.segFor(bid); return m ? (m.dur || 0) : 0; },
      audioBust() {
        const src = this.paper && this.paper.audio && this.paper.audio.src;
        return (src && this.store && this.store.assetBust) ? (this.store.assetBust[src] || 0) : 0;
      },
      /* Dirty state for a meta field (title/subtitle/abstract), tracked under
         paper.field_edits so it shows the same "edited" flag as blocks. */
      fieldDirty(field) {
        const fe = this.paper && this.paper.field_edits && this.paper.field_edits[field];
        return !!(fe && fe.dirty);
      },
      dirtyCount() { return (this.edit && this.edit.dirtyCount) ? this.edit.dirtyCount() : 0; },
      regenChanged() { if (this.edit && this.edit.regenerateChangedAudio) this.edit.regenerateChangedAudio(); },
    ...
    template: `
      <article class="civic-detail" :class="{ 'toc-open': tocOpen }"
        ...
        <button class="toc-fab"
                ...
                :aria-label="(tocOpen ? ((store.t.ui && store.t.ui.close_contents) || 'Close contents') : ((store.t.ui && store.t.ui.contents) || 'Contents'))">
        ...
        <nav class="col-toc"
          ...
          <button class="back"
                  @click="$emit('back')"
                  :aria-label="((store.t.ui && store.t.ui.back_to_library_full) || 'Back to the library')">
        ...
        <section class="col-doc">
          <div class="doc-inner">
            <div class="doc-meta">
              ...
              <span>{{ (store.t.ui && store.t.ui.tier_labels && store.t.ui.tier_labels[paper.tier]) || paper.tier }}</span>
              <span>{{ paper.sequence }}</span>
            </div>
            <h1 class="doc-title" :id="'paper-title-' + paper.id"><editable-text tag="span" :obj="paper" field="title" /></h1>
            <div v-if="edit && edit.enabled" class="vw-field-audio" :class="{ 'vw-field-dirty': fieldDirty('title') }">
              <span class="vw-field-lbl">title</span>
              <block-audio :src="segSrc('title')" :dur="segDur('title')" :bust="audioBust()" />
              <button class="vw-seg-regen" :class="{ 'vw-seg-regen-hot': fieldDirty('title') }" @click="edit.regenerateSegment('title')" :disabled="edit.saving" title="Regenerate the title audio">↻</button>
              <span v-if="fieldDirty('title')" class="vw-seg-dirty" title="Edited since last audio — needs regeneration">edited</span>
            </div>
            <div class="doc-sub"><editable-text tag="span" :obj="paper" field="subtitle" /></div>
            <div v-if="edit && edit.enabled" class="vw-field-audio" :class="{ 'vw-field-dirty': fieldDirty('subtitle') }">
              <span class="vw-field-lbl">subtitle</span>
              <block-audio :src="segSrc('subtitle')" :dur="segDur('subtitle')" :bust="audioBust()" />
              <button class="vw-seg-regen" :class="{ 'vw-seg-regen-hot': fieldDirty('subtitle') }" @click="edit.regenerateSegment('subtitle')" :disabled="edit.saving" title="Regenerate the subtitle audio">↻</button>
              <span v-if="fieldDirty('subtitle')" class="vw-seg-dirty" title="Edited since last audio — needs regeneration">edited</span>
            </div>
            ...
            <div class="doc-byline">
              ...
              <div class="field">
                ...
                <div class="v">{{ (store.t.ui && store.t.ui.status_labels && store.t.ui.status_labels[paper.status]) || paper.status }}</div>
              </div>
              <div class="field" v-if="paper._meta && paper._meta.updated">
                <div class="l">{{ (store.t.ui && store.t.ui.updated) || 'Updated' }}</div>
                <div class="v">{{ paper._meta.updated }}</div>
              </div>
              <div class="field" v-if="paperRepos.length">
                <div class="l">{{ paperRepos.length > 1 ? ((store.t.ui && store.t.ui.repositories) || 'Repositories') : ((store.t.ui && store.t.ui.repository) || 'Repository') }}</div>
                <div class="v">
                  <template v-for="(r, i) in paperRepos" :key="r.url"><a v-if="r.available && !r.private" :href="r.url" target="_blank" rel="noopener">{{ r.name }}</a><span v-else class="repo-pending">{{ r.name }}</span><span v-if="i < paperRepos.length - 1"> · </span></template>
                </div>
              </div>
            </div>
            <details class="doc-changelog" v-if="paper._meta && paper._meta.changelog && paper._meta.changelog.length"
              ...
              <summary style="cursor:pointer;font-family:var(--font-mono);font-size:11px;letter-spacing:0.06em;text-transform:uppercase;color:var(--accent);">
                {{ (store.t.ui && store.t.ui.what_changed) || 'What changed' }} ({{ paper._meta.changelog.length }})
              </summary>
              ...
              <div class="vw-meta-row">
                <label>Primary locale (canonical structure)
                  <select :value="paper.primary_locale || 'en'" @change="edit.setPrimaryLocale($event.target.value)">
                    <option v-for="l in store.site.locales" :key="l.code" :value="l.code">{{ l.label }}</option>
                  </select>
                </label>
            ...
            <div v-if="store.locale !== (paper.primary_locale || 'en') && paper.translation_status && paper.translation_status !== 'final'"
            ...
            <div class="abstract" :class="{ 'vw-field-dirty': edit && edit.enabled && fieldDirty('abstract') }">
              <div class="lbl">{{ (store.t.ui && store.t.ui.abstract) || 'Abstract' }}</div>
              <p><editable-text tag="span" :obj="paper" field="abstract" /></p>
              <div v-if="edit && edit.enabled" class="vw-field-audio">
                <span class="vw-field-lbl">abstract</span>
                <block-audio :src="segSrc('abstract')" :dur="segDur('abstract')" :bust="audioBust()" />
                <button class="vw-seg-regen" :class="{ 'vw-seg-regen-hot': fieldDirty('abstract') }" @click="edit.regenerateSegment('abstract')" :disabled="edit.saving" title="Regenerate the abstract audio">↻</button>
                <span v-if="fieldDirty('abstract')" class="vw-seg-dirty" title="Edited since last audio — needs regeneration">edited</span>
              </div>
            </div>
            ...
            <div class="body">
              <template v-for="(b, i) in paper.blocks" :key="blockKey(b)">
                <div v-if="edit && edit.enabled" class="vw-block" :class="{ 'vw-block-dirty': b.dirty }" :data-block-type="b.type">
                  <div class="vw-block-bar">
                    <span class="vw-block-type">{{ b.type }}</span>
                    <block-audio v-if="['section_heading','paragraph','dropcap_paragraph','pullquote','sidenote'].includes(b.type)"
                                 :src="segSrc(b.bid)" :dur="segDur(b.bid)" :bust="audioBust()" />
                    <button v-if="['section_heading','paragraph','dropcap_paragraph','pullquote','sidenote'].includes(b.type)"
                            class="vw-seg-regen" :class="{ 'vw-seg-regen-hot': b.dirty }"
                            @click="edit.regenerateSegment(b.bid)" :disabled="edit.saving"
                            title="Regenerate this block's audio">↻</button>
                    <span v-if="b.dirty" class="vw-seg-dirty" title="Edited since last audio — needs regeneration">edited</span>
        ...
        <div v-if="edit && edit.enabled && dirtyCount() > 0" class="vw-regen-bar">
          <span class="vw-regen-count">{{ dirtyCount() }} block{{ dirtyCount() === 1 ? '' : 's' }} edited</span>
          <button class="vw-gen-btn" @click="regenChanged" :disabled="edit.saving">
            {{ edit.saving ? 'Working…' : 'Regenerate changed audio' }}
          </button>
          <button class="vw-edit-btn vw-stitch-btn" @click="edit.stitchAudio()" :disabled="edit.saving"
                  title="Rejoin segments into the full narration (no new synthesis)">Stitch</button>
          <span v-if="edit.status" class="vw-regen-status">{{ edit.status }}</span>
        </div>
      </article>
    `,
  };
