...
(function () {
  ...
  /* Locale metadata, driven by data/site.json so SEO codes and hreflang
     alternates cover every declared language automatically. */
  ...
  V.setPaper = function (paper) {
    ...
    /* Each paper has a tailored social card (its title on the collection
       ...
       Non-default locales get their own translated card at <id>.<locale>.jpg. */
    const ogLoc = d.locale;
    const card = SITE_URL_BASE + '/public/og/' + paper.id + (ogLoc && ogLoc !== defaultLoc() ? '.' + ogLoc : '') + '.jpg';
