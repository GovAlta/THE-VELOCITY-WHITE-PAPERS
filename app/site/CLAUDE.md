...
**Read `../../style-guide/voice-exemplar.md` first.** It is the primary positive reference and shows the author's real voice: long, flowing, comma-joined, first person plural, specifics woven in. The rules below are a **filter on prose you generate from scratch** — what not to add. They are **not** a license to rewrite the author's own sentences. When the author has written the words, his words are the source of truth: fix what is false or accidental, preserve his cadence. Chopping his flowing sentences into short declaratives to satisfy the rules below produces a different kind of AI smell and is the failure these notes exist to prevent.

**Never emit a phrase you recognise from a style guide's "Forbidden" column.** The guides quote bad writing to warn you off it. Two such phrases ("Velocity is an imperative," "the weight underneath is the reason") were once pasted into wp-08 as actual content. See the poison-phrase list in `voice-exemplar.md`.
